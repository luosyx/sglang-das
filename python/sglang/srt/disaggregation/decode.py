...
class SchedulerDisaggregationDecodeMixin:
        ...
        while True:
            ...
            if self._engine_paused:
                ...
                if getattr(self, "require_mlp_sync", False):
                    self.dp_attn_adapter.maybe_prepare_mlp_sync_batch(None)
                continue
    ...
    def process_decode_queue(self: Scheduler):
        ...
        try:
            ...
            # Try to resume retracted requests if there is enough space for another
            # `num_reserved_decode_tokens` decode steps.
            resumed_reqs = self.disagg_decode_prealloc_queue.resume_retracted_reqs()
            self.waiting_queue.extend(resumed_reqs)
            if len(self.disagg_decode_prealloc_queue.retracted_queue) > 0:
                # still retracted requests pending: do not allocate new ones
                return

            if not hasattr(self, "polling_count"):
                self.polling_count = 0
                self.polling_interval = (
                    get_disagg().disaggregation_decode_polling_interval
                )
